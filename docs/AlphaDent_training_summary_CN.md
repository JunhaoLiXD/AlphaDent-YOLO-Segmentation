...
2. **两阶段 detect-then-refine 流程** —— 进行中的实验
   ...
   - 第二阶段：训练好的精修器（U-Net + ImageNet ResNet18）对小框 crop 在原分辨率下重新分类+重新分割；
     大框直接走 V6（V13 的护栏）。
   - **状态（2026-06-17）：** Phase 0 oracle 验证了天花板（小 Caries +0.11..+0.22，`src/04`）；
     Phase 1a 门槛**通过**（V6 小 Caries 召回 ≈0.58–0.89 @conf0.05），但 Phase 1b 迁移**弱**
     （`full@0.05`=0.182 < V6 0.210，缺背景类；`src/05`）。Phase 1c（`src/06`，在真实 V6 框上重训
     + 加背景类、热启动）**已搭建，尚未训练**。
