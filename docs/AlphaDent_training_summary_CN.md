...
**挂起**。真正撬动分数的是全类 **V6+V10 ensemble + hflip TTA**(公榜 0.31189 —— 见 §7)。
...
**最佳提交：V6+V10 ensemble + hflip TTA（公榜 0.31189）——见 §7。** 下面汇总的是它所基于的最佳
*单模型*。
...
## 7. V6+V10 ensemble + hflip TTA —— 首个排行榜增益（公榜 0.31189）
...
**排行榜。** `0.31189` 对比单 V6 `0.27047` → **+0.0414**，首个超过单模型 V6 的提交。公榜涨幅约为 val
可比指标增量（+0.008）的 5 倍——**本地指标明显低估了真实增益**，应把它当作保守的方向性信号，而非绝对值。

**状态。** 这是**生产提交**，**零额外训练**。`src/09` 是 val 增益验证；`src/10` 是提交构建器（自动识别
V6/V10 + 竞赛 test 集，在 val 上调阈值，写出 `id,patient_id,class_id,confidence,poly` 格式的
`submission.csv`）。还没花的廉价后续：降低 `ENS_NMS_IOU`、加 TTA 视图（vflip/多尺度）、对两模型置信度
加权、或更大 backbone（yolov8m/l-seg @768）。